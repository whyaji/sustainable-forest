...
// === FACTORY ===
const getGroupActivities = factory.createHandlers(async (c) => {
...
const getImages = factory.createHandlers(async (c) => {
  return await getPaginationData({
    c,
    table: groupActivitySchema,
    searchBy: 'title',
...
const getGroupActivityById = factory.createHandlers(async (c) => {
...
const createGroupActivity = factory.createHandlers(async (c) => {
...
const updateGroupActivity = factory.createHandlers(async (c) => {
  const id = c.req.param('id');
  const formData = await c.req.parseBody();
...
const deleteGroupActivity = factory.createHandlers(async (c) => {
  const id = c.req.param('id');
  const existing = await db
    .select()
    .from(groupActivitySchema)
...
export const groupActivityHandler = {
  getGroupActivities,
  getImages,
